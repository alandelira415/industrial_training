...
int main(int argc, char** argv)
{
  ...
  ros::Publisher cloud_pub = nh.advertise<pcl::PointCloud<pcl::PointXYZ>>(argv[1], 1, true);
  ...
  for (it2; it2 != linkMap.end(); it2++)
  {
    ...
    if ((linkPtr && linkPtr->visual && linkPtr->visual->geometry) && (linkPtr && linkPtr->visual))
    {
      ...
      if (tmp && tmp2)
      {
        ...
        // dae files are still broken as of 8/21/18. The internal transforms are not properly imported by assimp in
        // gl_depth_sim
  ...
  std::vector<unsigned int> indices = { 0, 2, 1, 0, 3, 2, 2, 3, 4, 2, 4, 5, 1, 2, 5, 1, 5, 6,
  ...
  while (ros::ok())
  {
    // The box currently doesn't move, so just run every second
    if ((ros::Time::now() - timer).toSec() > 1.0)
    {
      // Step 1: Get camera tranform
      tf::StampedTransform camera_transform;
      try
      {
        listener.lookupTransform("/world", camera_frame, ros::Time(0), camera_transform);
      }
      ...
      //    pose.rotate(Eigen::AngleAxisd(M_PI/2, Eigen::Vector3d(0,1,0))); // currently done in launch file with
      //    seperate TF

      // Step 1.5: Get transform of box and move mesh.

      // Step 2: Render point cloud if needed
      if (update)
      {
        depth_img = sim.render(pose);
        update = false;
      }

      // Step 3: Publish the cloud
